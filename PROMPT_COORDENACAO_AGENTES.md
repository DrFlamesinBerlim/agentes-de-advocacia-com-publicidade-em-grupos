...
1. **Estado Consolidado (`sync_state.json`):** 
   Toda máquina local executa um monitor em segundo plano (`loop_monitor.py`) a cada **5 minutos (300 segundos)**. Esse monitor calcula o hash SHA-256 e o timestamp de modificação de cada arquivo e atualiza o estado consolidado.
   
2. **Escrita Atômica:**
   Qualquer alteração feita por você nos arquivos da pasta deve ser feita de forma **atômica** (escrever num arquivo `.tmp` e depois renomear/sobrescrever o original) para evitar que o monitor de 5 minutos (300 segundos) de outra máquina tente ler o arquivo pela metade.
